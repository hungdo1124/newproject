...
// ============================================================
// 🛠️ FIX LOGIC ROUTER: HỖ TRỢ CẢ CÓ /API VÀ KHÔNG CÓ /API
// ============================================================

// 1️⃣ AUTH ROUTER (Nhóm chức năng đăng nhập, đăng ký, OTP)
...
authRouter.post('/login', async (req, res) => {
    try {
        ...
        const userData = { ...u, password: undefined, otp: undefined, hasSecurityCode: !!u.securityCode };
...
authRouter.post('/register', async (req, res) => {
    try {
        ...
        const newUser = {
            ...
            role: "user", isVerified: false, otp,
...
authRouter.post('/verify-otp', async (req, res) => {
    // Logic xác thực xong và kích hoạt tài khoản
    try {
        const { email, otp } = req.body;
        const users = await readFile(USERS_FILE);
        const u = users.find(x => x.email === email);
...
// 2️⃣ USER ROUTER (Nhóm chức năng cá nhân)
...
userRouter.put('/security-code', async (req, res) => {
    try {
        const { securityCode } = req.body;
        if (!securityCode || securityCode.length < 4) return res.status(400).json({ message: "Mã >= 4 ký tự" });
        const users = await readFile(USERS_FILE);
        const u = users.find(u => u.id === req.user.id);
        u.securityCode = await bcrypt.hash(securityCode, 10);
        await writeFile(USERS_FILE, users);
        res.json({ message: "Đã lưu mã" });
    } catch (e) { res.status(500).json({ message: "Err" }); }
});

userRouter.post('/verify-security', async (req, res) => {
    try {
        const { securityCode } = req.body;
        const users = await readFile(USERS_FILE);
        const u = users.find(u => u.id === req.user.id);
        if (!u.securityCode) return res.status(400).json({ message: "Chưa tạo mã" });
        const isMatch = await bcrypt.compare(securityCode, u.securityCode);
...
// 🔥 QUAN TRỌNG: Gắn Router vào CẢ 2 ĐƯỜNG DẪN (Có và Không có /api)
// Để web gọi kiểu gì cũng chạy được
app.use('/api/auth', authRouter); // Cho trang Login (Đang chạy đúng)
app.use('/auth', authRouter);     // Cho trang Check OTP (Đang bị lỗi)

app.use('/api/user', authenticateToken, userRouter); // Cho chuẩn
app.use('/user', authenticateToken, userRouter);     // Cho trang cá nhân (Đang bị lỗi)

// --- ADMIN & POSTS ROUTES (Giữ nguyên) ---
...
const startServer = async () => {
    ...
    if (!fs.existsSync(USERS_FILE)) {
        ...
        const adminUser = [{ id: "admin001", name: "Administrator", email: "admin@newsdaily.com", password: hashedPassword, isVerified: true, role: "admin", avatar: "https://ui-avatars.com/api/?name=Admin" }];
        await writeFile(USERS_FILE, adminUser);
    }
    if (!fs.existsSync(POSTS_FILE)) await writeFile(POSTS_FILE, []);
